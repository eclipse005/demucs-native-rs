...
//!
//! GPU kernels are compiled offline with `nvcc -ptx` (see
//! `scripts/compile-ptx.ps1`) and embedded here. At runtime we pick the highest
//! prebuilt arch that is ≤ the device's compute capability. This removes any
//! need for NVRTC / a CUDA Toolkit on end-user machines — only the driver plus
//! the cudart / cuBLAS runtime DLLs are required.
//!
//! PTX is forward-compatible: a newer device JITs the newest PTX whose target
//! is ≤ its own compute capability, so `sm_90` also covers Blackwell parts.
...
pub fn resolve_ptx_for_device(major: i32, minor: i32) -> Result<(&'static str, u32), String> {
    ...
    let selected = select_prebuilt_sm(device_sm).ok_or_else(|| {
        format!("GPU compute capability sm_{device_sm} is below the minimum supported sm_{MIN_SM}")
    })?;
...
mod tests {
    ...
    /// GPU smoke: load prebuilt PTX through the same path `CudaState` uses.
    ...
    fn load_prebuilt_ptx_on_device_if_available() {
        ...
        // Touch a kernel the engine actually resolves at startup.
        let _ = module
            .load_function("noop_placeholder")
            .expect("load noop_placeholder from prebuilt PTX");
    }
}
