//! Test conv2d_3x3_s1p1 with actual hdec rewrite weights.
...
#[test]
#[ignore]
fn cuda_conv2d_3x3_hdec3_matches_cpu() {
    ...
    let c_in = layer.rewrite.in_ch;
    let fr_in = 16;
    let t = 32;
    let b = 1;
    let input: Vec<f32> = (0..b*c_in*fr_in*t).map(|i| ((i as f32)*0.013 - 0.3)*0.5).collect();

    let (cpu_out, cpu_shape) = ops_cpu::conv2d(
        &input, [b, c_in, fr_in, t],
    ...
    let gpu_in = state.upload_f32(&input, vec![b, c_in, fr_in, t]).expect("up");
    let gpu_out: GpuTensor = demucs_core_native::cuda_ops::conv2d_3x3_s1p1(&state, &gpu_in, &gpu_w, &gpu_b).expect("conv");
